...
etranslate (Apply (Variable f) es) = 
  case in_init_env f of
  True -> Apply (Variable f) (map etranslate es)
  False -> Apply (Variable f) (map (ref.etranslate) es)

etranslate (Variable x) = case in_init_env x of
  True -> (Variable x)
  False -> (Apply (Variable "!") [(Variable x)] )
...
dtranslate (Val x e) = Val x ( (ref.etranslate) e ) --(Let (Val x (fresh mem))  (Assign x (etranslate e)) ) x
--val x=ref(e ] )
-- other possibilities: preserve the structure
...
obey (Calculate exp) (env, mem) =
  let (v, mem') = eval (etranslate exp) env mem in
  (print_value v, (env, mem'))

obey (Define def) (env, mem) =
  let x = def_lhs def in
  let (env', mem') = elab (dtranslate def) env mem in
